...
int	call_macho(void *bin, size_t off_set2, size_t s, char **av)
{
		...
		off_set2) == MH_CIGAM)
	{
		...
		printf("ppc):\nContents of (__TEXT,__text) section\n") :
		printf("i386):\nContents of (__TEXT,__text) section\n");
		if (ft_otool_32(bin + off_set2, *(struct mach_header *)(bin +
...
fat_arch_64 *)arch).offset) : (*(struct fat_arch_64 *)arch).offset;
	...
	if (call_macho(bin, off_set2, s, av))
		return (1);
	return (0);
}
...
int	fat_bin(void *bin, size_t size, int nb_arch, char **av)
{
	...
	while (++x < nb_arch)
	{
		...
		off_set >= size ? printf("Binary corrupted!\n") : 0;
		if (off_set >= size)
			return (1);
	}
	return (0);
