...
void	print_64(size_t *x, struct section_64 section, void *bin, int end)
{
	if (!end)
	{
		(*x - section.offset) % 16 == 15 && *x - section.offset < section.
size ? printf("%02x \n%016llx\t", *(unsigned char *)(bin + *x), (section.addr +
...
size ? printf("%08x \n%016llx\t", r_int32(*(uint *)(bin + *x)), (section.addr +
*x + 4 - section.offset)) : printf("%08x ", r_int32(*(uint *)(bin + *x)));
		*x += 4;
	}
}
